...
// Generate demo session data for testing
export async function generateDemoSession(): Promise<SessionSummary> {
  const emotions = ['anxious', 'stressed', 'neutral', 'calm', 'hopeful'];
  const timeline: EmotionTimestamp[] = [];
  const duration = Math.floor(Math.random() * 600) + 300; // 5-15 min
  
  // Generate emotional journey
  let currentIndex = Math.floor(Math.random() * 3); // Start negative to neutral
  for (let t = 0; t < duration; t += 30) {
    // Tendency to improve over time
    if (Math.random() > 0.6 && currentIndex < emotions.length - 1) {
      currentIndex++;
    } else if (Math.random() < 0.2 && currentIndex > 0) {
      currentIndex--;
    }
  ...
  const techniques = ['4-7-8 Breathing', 'Box Breathing', 'Cognitive Restructuring', 'STOP Skill', 'Body Scan'];
  const coachingMoments: CoachingMoment[] = [];
  
  // Add some coaching moments
  for (let i = 0; i < 4; i++) {
    coachingMoments.push({
      time: Math.floor(Math.random() * duration),
      type: 'technique',
      content: `Let's try ${techniques[i]}`,
      techniqueName: techniques[i],
    });
  }
  
  const startTime = new Date();
  startTime.setMinutes(startTime.getMinutes() - Math.floor(duration / 60));
